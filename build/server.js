...
export function initServer(client) {
    ...
    server.tool("get-spotify-profile", "Get the current user's Spotify profile", {}, async () => {
        try {
    ...
    server.tool("get-spotify-playlist-items", "Get items from a specific Spotify playlist", {
    ...
    server.tool("get-spotify-playlists", "Get the current user's Spotify playlists", {
        limit: z
            .number()
            .min(1)
            .max(50)
    ...
    // Search Spotify tool
    server.tool("search-spotify", "Search for content on Spotify", {
        ...
        try {
            const results = await client.search(query, types, limit, offset);
            return {
                content: [
                    {
                        type: "text",
